---
title: "Projekt"
author: "Luka Kovačević, Bruno Pleše, Lucija Prodan, Dominik Topić"
date: "15.12.2025."
output:
  word_document: default
  pdf_document: default
subtitle: Case study *Predikcija moždanog udara*
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Motivacija i opis problema

Moždani udar jedna je od vodećih uzroka smrti i invaliditeta širom svijeta. Prema podacima Svjetske zdravstvene organizacije (WHO), moždani udar odgovoran je za približno 11% svih smrtnih slučajeva globalno. Rana identifikacija osoba s visokim rizikom od moždanog udara ključna je za pravovremenu intervenciju i prevenciju.

Suvremeni pristupi u medicini sve više koriste statističke metode i strojno učenje za analizu zdravstvenih podataka kako bi se identificirali čimbenici rizika i razvili prediktivni modeli. Ovaj projekt fokusira se na analizu kliničkih podataka pacijenata s ciljem razumijevanja čimbenika koji doprinose riziku od moždanog udara.

## Case study: *Predikcija moždanog udara*

Interpretacija podataka značajan je dio obrade podataka. Stoga je bitno znati kontekst podataka i upoznati se s datasetom.

```{r}
stroke_data = read.csv("healthcare-dataset-stroke-data.csv")
dim(stroke_data)

```

```{r}
head(stroke_data)
```

## Osnovna deskriptivna statistika

Cilj deskriptivne statistike je opisivanje, sažimanje i interpretacija podataka na način koji olakšava razumijevanje njihovih osnovnih obilježja, ali ne daje nikakve zaključke o uzrocima ili posljedicama.

```{r}
# Osnovna deskriptivna statistika:
summary(stroke_data)
```

Naredba summary() koristi se za brz pregled osnovnih statističkih karakteristika podataka (raspon, medijan, prosjek i kvartile).

Skup podataka sadrži 5110 pacijenata. Dob pacijenata kreće se od 0.08 do 82 godine, s prosječnom dobi od 43.23 godine i medijanom od 45 godina. Većina pacijenata nema hipertenziju ni srčane bolesti, što je vidljivo iz niskih prosječnih vrijednosti binarnih varijabli hypertension (9.7%) i heart_disease (5.4%).

Prosječna razina glukoze iznosi 106.15 mg/dL, pri čemu su prisutne izrazito visoke maksimalne vrijednosti, što upućuje na asimetričnu distribuciju. Ciljna varijabla stroke ima prosječnu vrijednost 0.0487, što znači da je oko 4.9% pacijenata doživjelo moždani udar, pa je skup podataka neuravnotežen.

Kategorijske varijable su učitane kao tip character, što zahtijeva njihovu konverziju u faktore prije daljnje analize.

```{r}
stroke_data$gender <- as.factor(stroke_data$gender)
stroke_data$ever_married <- as.factor(stroke_data$ever_married)
stroke_data$work_type <- as.factor(stroke_data$work_type)
stroke_data$Residence_type <- as.factor(stroke_data$Residence_type)
stroke_data$smoking_status <- as.factor(stroke_data$smoking_status)
stroke_data$stroke <- as.factor(stroke_data$stroke)

```

Naredba table() koristi se za prikaz frekvencija kategorijskih varijabli.

```{r}
lapply(stroke_data[, sapply(stroke_data, is.factor)], table)
```

U uzorku prevladavaju žene (2994) u odnosu na muškarce (2115), dok je kategorija Other zanemariva. Većina pacijenata je u braku (3353), a najzastupljeniji tip zaposlenja je Private sektor (2925), dok je najmanje pacijenata koji nikada nisu radili.

Distribucija prebivališta je gotovo ravnomjerna između urbanih (2596) i ruralnih (2514) područja. Kod statusa pušenja najčešća je kategorija never smoked (1892), dok je značajan udio pacijenata s nepoznatim statusom pušenja (1544), što može utjecati na daljnju analizu.

```{r}
table(stroke_data$stroke)
prop.table(table(stroke_data$stroke)) * 100
```

Ciljna varijabla stroke pokazuje izrazitu neuravnoteženost, s velikom većinom pacijenata bez moždanog udara (4861) u odnosu na one koji su ga doživjeli (249).

## Čišćenje podataka

Moguće je da u podatcima nedostaju neke vrijednosti, koje u učitanom dataframe-u poprimaju vrijednost `N/A`.

Kako ćemo tretirati nedostajuće vrijednosti ovisit će prvenstveno o samom datasetu, odnosno kontekstu podataka.

```{r}
colSums(is.na(stroke_data))
```

Naredba colSums(is.na()) koristi se za utvrđivanje broja nedostajućih vrijednosti po varijabli.

```{r}
table(stroke_data$bmi)
```

```{r}
stroke_data$bmi[stroke_data$bmi == "N/A"] <- NA
stroke_data$bmi <- as.numeric(stroke_data$bmi)
```

```{r}
colSums(is.na(stroke_data))
```

Iako je u početnom prikazu izgledalo da nema nedostajućih vrijednosti, varijabla bmi sadržavala je vrijednosti "N/A" učitane kao znakovni tip podataka, koje R ne prepoznaje kao stvarne nedostajuće vrijednosti. Nakon konverzije "N/A" u NA, utvrđeno je da varijabla bmi sadrži 201 nedostajuću vrijednost.

```{r}
imp_data <- stroke_data

# Stvaramo regresijski model prema kojem ćemo previđati BMI
bmi_imp_model <- lm(bmi ~ age + gender + avg_glucose_level + heart_disease + hypertension,
                    data = imp_data,
                    na.action = na.exclude)

# Previđamo vrijednosti
bmi_pred <- predict(bmi_imp_model, newdata = imp_data)

# Uzimamo standardnu devijaciju residuala
res_sd <- sd(residuals(bmi_imp_model), na.rm = TRUE)

# Prepisujemo postojeći BMI u novi stupac
imp_data$bmi_imp <- imp_data$bmi

# U novom stupcu imputiramo nedostajeće BMI vrijednosti koristeći predviđene BMI
# vrijednosti i dodajemo varijancu pomoću standardne devijacije residuala
imp_data$bmi_imp[is.na(imp_data$bmi)] <- bmi_pred[is.na(imp_data$bmi)] + rnorm(sum(is.na(imp_data$bmi)), 0, res_sd)


# Stvaramo graf za usporedbu
par(mfrow = c(1,2))
hist(imp_data$bmi, main = "Izvorni BMI", col = "gray", xlab = "BMI")
hist(imp_data$bmi_imp, main = "Imputirani BMI", col = "blue", xlab = "BMI")

# Prepisujemo N/A vrijednosti izračunatim
stroke_data$bmi[is.na(stroke_data$bmi)] <- imp_data$bmi_imp[is.na(stroke_data$bmi)]
```

S obzirom da se veliki udio N/A vrijednosti nalazi među pacijentima s moždanim udarom, kojih je neuravnoteženo manje od onih koji nisu doživjeli moždani udar, odabrali smo stohastičku regresijsku imputaciju budući da zadržava relativne odnose među čimbenicima te uključuje i varijabilnu komponentu pomoću reziduala regresije.

## 1. pitanje: Postoji li statistički značajna razlika u prosječnoj razini glukoze između pacijenata sa i bez moždanog udara?

Moždani udar jedan je od vodećih uzroka smrtnosti i trajnih posljedica za zdravlje, a rizični čimbenici poput povišene razine glukoze u krvi često se povezuju s njegovom pojavom. Razina glukoze može odražavati metaboličke poremećaje poput dijabetesa, koji su poznati čimbenici rizika za kardiovaskularne bolesti.

Cilj ovog zadatka je ispitati postoji li statistički značajna razlika u prosječnoj razini glukoze između pacijenata koji su doživjeli moždani udar i onih koji ga nisu doživjeli. Time se nastoji procijeniti može li razina glukoze poslužiti kao potencijalni pokazatelj povećanog rizika od moždanog udara, što ima važno značenje u prevenciji i ranom prepoznavanju rizičnih skupina pacijenata.

Kako bi se ispitala razlika u prosječnoj razini glukoze između pacijenata sa i bez moždanog udara, postavljene su sljedeće hipoteze:

H0: Ne postoji razlika u prosječnoj razini glukoze između pacijenata sa i bez moždanog udara. \
H1: Postoji razlika u prosječnoj razini glukoze između pacijenata sa i bez moždanog udara.

Prije provođenja statističkog testa, prikazana je deskriptivna statistika prosječne razine glukoze za pacijente sa i bez moždanog udara, kako bi se stekao uvid u osnovne razlike između promatranih skupina.

```{r}
tapply(
  stroke_data$avg_glucose_level,
  stroke_data$stroke,
  summary
)

```

Deskriptivna statistika pokazuje jasne razlike između promatranih skupina. Pacijenti bez moždanog udara imaju prosječnu razinu glukoze od 104.8 mg/dL i medijan od 91.5 mg/dL, dok pacijenti s moždanim udarom imaju znatno višu prosječnu razinu glukoze od 132.5 mg/dL i medijan od 105.2 mg/dL. Također je vidljivo da su više kvartilne vrijednosti i maksimalne razine glukoze izraženije u skupini pacijenata s moždanim udarom, što upućuje na veći raspon i varijabilnost u toj skupini. To možemo i vizualizirati.

```{r}
boxplot(
  avg_glucose_level ~ stroke,
  data = stroke_data,
  col = c("lightblue", "tomato"),
  main = "Razine glukoze prema moždanom udaru",
  xlab = "Moždani udar",
  ylab = "Prosječna razina glukoze (mg/dL)"
)
```

Velik broj stršećih vrijednosti posljedica je asimetrične distribucije razine glukoze i velikog uzorka, a ne pogrešaka u podacima, stoga nisu uklonjene iz analize.

```{r}
glucose_0 <- stroke_data$avg_glucose_level[stroke_data$stroke == 0]
glucose_1 <- stroke_data$avg_glucose_level[stroke_data$stroke == 1]

par(mfrow = c(1, 2))

hist(glucose_0,
     breaks = 30,
     col = "lightblue",
     main = "Glukoza (bez moždanog udara)",
     xlab = "Prosječna razina glukoze (mg/dL)")

hist(glucose_1,
     breaks = 30,
     col = "tomato",
     main = "Glukoza (moždani udar)",
     xlab = "Prosječna razina glukoze (mg/dL)")

par(mfrow = c(1, 1))

```

Vidljivo je da je distribucija za pacijente s moždanim udarom pomaknuta prema višim vrijednostima glukoze, dok pacijenti bez moždanog udara češće imaju niže i srednje vrijednosti. Histogrami razine glukoze za obje skupine pokazuju izraženu desnu asimetriju, pri čemu manji broj pacijenata ima znatno povišene vrijednosti glukoze. Ovakav oblik distribucije posebno je vidljiv u skupini pacijenata s moždanim udarom.

```{r}
par(mfrow = c(1, 2))

qqnorm(glucose_0, main = "QQ-plot glukoze (bez moždanog udara)")
qqline(glucose_0, col = "red")

qqnorm(glucose_1, main = "QQ-plot glukoze (moždani udar)")
qqline(glucose_1, col = "red")

par(mfrow = c(1, 1))

```

QQ-plotovi dodatno potvrđuju odstupanja od normalne distribucije, osobito u gornjim kvantilima, gdje se uočava značajno odstupanje točaka od referentne linije. To upućuje na prisutnost asimetrije i ekstremnih vrijednosti u obje skupine.

```{r}
library(nortest)
```

Za formalnu provjeru normalnosti koristi se Lilliefors test, koji ispituje nul-hipotezu da podaci dolaze iz normalne distribucije. Ovo su hipoteze testa: H0: Podaci dolaze iz normalne distribucije H1: Podaci ne dolaze iz normalne distribucije

```{r}
lillie.test(glucose_0)
lillie.test(glucose_1)

```

Lillieforsov test odbacuje nul-hipotezu normalnosti, što ukazuje da distribucija razine glukoze odstupa od normalne distribucije. Budući da smo vizualno i formalno utvrdili da distribucija razine glukoze značajno odstupa od normalne, odlučujemo se za neparametarski test koji ne zahtijeva normalnost: Mann-Whitney-Wilcox test. Postavljamo hipoteze: H0: Prosječne razine glukoze jednake su za pacijente sa i bez moždanog udara. H1: Prosječne razine glukoze razlikuju se između pacijenata sa i bez moždanog udara.

```{r}
wilcox.test(
  avg_glucose_level ~ stroke,
  data = stroke_data,
  exact = FALSE
)
```

Mann-Whitney-Wilcoxonov test pokazuje da postoji statistički značajna razlika u prosječnim razinama glukoze između pacijenata sa i bez moždanog udara (p = 3.64 × 10⁻⁹). Time se odbacuje nul-hipoteza jednakosti, što upućuje na to da se tipične vrijednosti razine glukoze razlikuju između promatranih skupina.

Dobiveni rezultati u skladu su s medicinskim spoznajama prema kojima povišena razina glukoze predstavlja važan rizični čimbenik za nastanak moždanog udara. Time analiza potvrđuje da se obrasci prisutni u stvarnim kliničkim podacima odražavaju i u promatranom skupu podataka, što dodatno naglašava relevantnost razine glukoze kao potencijalnog pokazatelja povećanog rizika od moždanog udara.



## 2. Pitanje: Postoji li interakcijski učinak hipertenzije i srčanih bolesti na BMI?



Analiziramo interakcijski učinak hipertenzije i srčanih bolesti na BMI, zanima nas utječe li kombinacija hipertenzije i srčanih bolesti dodatno na BMI, više od svake bolesti pojedinačno. 

Postavljamo hipoteze: 

  H0: Ne postoji interakcijski učinak hipertenzije i srčanih bolesti na BMI
  H1: Postoji interakcijski učinak hipertenzije i srčanih bolesti na BMI
  
Metode između kojih biramo kako bi napravili analizu su ANOVA i višestruka regresija. Za anovu nije zadovoljena normalnost ni homogenost varijance te zato ANOVA nije statistički valjana za ove podatke.

Koristit ćemo višestruku linearnu regresiju jer nam omogućuje istovremeno modeliranje hipertenzije i srčanih bolesti i njihove interakcije. Kako je BMI pozitivno asimetričan koristimo log-transformaciju kako bi osigurali približnu normalnost reziduala i stabilizirali varijancu, a grafičkim pregledom provjeravamo homogenost varijance.

Kako bi procijenili interakcijski učinak napravit ćemo 2 modela, jedan sa samo glavnim efektima i jedan koji ima i interakcijski član.


```{r}
#novi dataset sa samo nužnim vrijednostima

data <- stroke_data[ , c("bmi" , "hypertension" , "heart_disease")]

aggregate(
  bmi ~ hypertension + heart_disease,
  data = stroke_data,
  mean
)

aggregate(
  bmi ~ hypertension + heart_disease,
  data = stroke_data,
  median
)

aggregate(
  bmi ~ hypertension + heart_disease,
  data = stroke_data,
  sd
)

model <- lm(log(bmi) ~ hypertension * heart_disease, data = data)

model0 <- lm(log(bmi) ~ hypertension + heart_disease, data = data)

hist(residuals(model), col = "royalblue4")

plot(fitted(model), residuals(model))
abline(h = 0, col = "red")

```

Deskriptivna analiza nam pokazuje razlike u prosječnim i medijanskim vrijednostima BMI-ja između skupina definiranih hipertenzijom i srčanim bolestima. Uočava se da kombinacija obje dijagnoze ne rezultira proporcionalno većim BMI-jem, što upućuje na mogući interakcijski učinak. Standardna devijacija nam pokazuje nejednaku raspršenost podataka što nam potvrđuje da varijanca nije homogena, to dodatno opravdava korištenje regresije umjesto ANOVE.

```{r}

anova(model0, model)

summary(model)

qqnorm(residuals(model))
qqline(residuals(model))

```
QQ-plot ima manja odstupanja u repovima, ali većinom prati referentnu liniju, što zadovoljava pretpostavku normalnosti za primjenu linearne regresije.

Anova test nam govori da je interakcija statistički značajna. Model s interakcijom statistički značajno bolje opisuje podatke.

Hipertenzija i srčane bolesti imaju značajan učinak na BMI, a njihova interakcija je negativna. To ukazuje da je zajednički učinak manji od zbroja pojedinačnih učinaka. Model objašnjava mali dio varijance BMI te iako je interakcija značajna, potrebno je razmatrati i druge faktore koji utječu na BMI.

Zaključno, analiza pokazuje da između hipertenzije i srčanih bolesti postoji statistički značajna interakcija u utjecaju na BMI. Ovaj rezultat naglašava važnost razmatranja zajedničkog djelovanja rizičnih čimbenika, umjesto njihove izolirane analize pri interpretaciji zdravstvenih pokazatelja.

## 3. Pitanje: Koji čimbenici najbolje predviđaju nastanak moždanog udara kada razmatramo dob, razinu glukoze, BMI i hipertenziju?

Zanima nas kako različiti čimbenici utječu na nastanak moždanog udara, ovdje smo izlučili dob, prosječnu razinu glukoze u krvi, BMI i hipertenziju jer su svi biološki faktori koji bi mogli imati direktnu povezanost.

Kako je potrebno usporediti utjecaj više nezavisnih varijabli na zavisnu, ovdje ćemo provesti logističku regresiju. Razlog zašto koristimo logističku, a ne linearnu regresiju je što nam je zavisna varijabla (reakcija) u binarnom obliku.

Prije provedbe regresije potrebno je uvjeriti se da su podaci ispravno prilagođeni, logistička regresija ne zahtjeva normalnu razdiobu, ali je nužan linearni odnos numeričkih vrijednosti s log-izgledom moždanog udara. To ćemo pokazati grafički za dob, razinu glukoze i BMI.

```{r} 
library(dplyr)
library(ggplot2)

plotdata <- stroke_data

# Izrada funkcije za manju redundantnost koda
plotfunc <- function(var, plotdata) {
  plotdata <- plotdata %>%
    # Dodajemo novi stupac prema kojem ćemo podijelit podatke u decile, stupac iskazuje pripadnost decilu
    mutate(var_decile = ntile({{var}}, 10)) %>%
    # Grupiramo po decilima
    group_by(var_decile) %>%
    # Računamo prosječnu vrijednost za svaki decil te udio pacijenata s moždanim udarom po decilu
    summarise(mean_var = mean({{var}}), p_stroke = mean(as.integer(as.character(stroke)))) %>%
    # Dodajemo stupac koji iskazuje log izgled moždanog udara s time da pribrajamo 0.001 na vjerojatnost moždanog udara kako bismo izbjegli nulu
    mutate(log_odds = log((p_stroke + 0.001) / (1 - p_stroke + 0.001)))
  # Stavljamo na graf prosjek naše varijable i log izgled moždanog udara
  ggplot(plotdata, aes(x = mean_var, y = log_odds)) + 
    geom_point(size = 3, color = "blue") + 
    geom_smooth(method = "lm", se = FALSE, color = "red") + 
    labs(
      x = paste("Središnja vrijednost", deparse(substitute(var)), "po decilu"),
      y = "Log-izgled moždanog udara",
      title = paste("Grafička provjera linearnosti",  deparse(substitute(var)), "u log-izgled skali")
    ) + 
    theme_minimal()
}

# Pozivamo funkciju nad sva tri atributa
plotfunc(age, plotdata)
plotfunc(avg_glucose_level, plotdata)
plotfunc(bmi, plotdata)
```

Vidimo da za sva tri atributa se podaci većinom slažu s pravcem, to ukazuje na traženu linearnu vezu. Odstupanja su većinom na prvim i zadnjim točkama što je očekivano, kod BMI-ja uočavamo nešto veća odstupanja, ali još uvijek unutar očekivanja. To možemo interpretirati da za bmi znatno veći od medijana vidimo manji utjecaj na log izgled moždanog udara.

Za hipertenziju nije potrebna ovakva provjera jer je varijabla binarna pa je podjela u decile besmislena te je linearnost pretpostavljena.

Sad kad smo potvrdili uvjete za logističku regresiju možemo realizirati model. Već sada pomoću grafova možemo naslutiti da će dob imati veći izražaj u modelu zbog relativne stabilnosti varijance oko pravca dok će bmi imati manji zbog većih odstupanja.

Provodimo logističku regresiju s logit vezom nad varijablama dob, razina glukoze, BMI i hipertenzija.

```{r} 
#Pretvorba stroke u brojevni tip
reg_data <- stroke_data
reg_data$stroke <- as.integer(as.character(reg_data$stroke))

#Kreiranje modela
model <- glm(stroke ~ age + avg_glucose_level + bmi + hypertension,
             data = reg_data,
             family = binomial)

#Pregled rezultata
summary(model)
exp(coef(model))
```

Regresija pokazuje da su vrijednosti glukoze i dob imali najznačajniji utjecaj. Hipertenzija je također bila značajna, no ne na istoj razini. BMI jedini nema značajni utjecaj.

Konačno, vidimo koeficijente eksponirane za lakše razumijevanje, gledajući u ispis summary-a, povećanje svakog atributa za 1 očituje se zbrajanjem pripadne vrijednosti na log-izgled, ta skala je zato teško interpretabilna pa transformiramo dobivene vrijednosti. Eksponiranjem dobijemo jasniju metriku, vrijedi npr. da ako povećamo dob za 1 će se vjerojatnost moždanog udara za pripadnu vrijednost umnožiti. Ovdje nam je i dalje teško usporediti varijable jer je svaka na svojoj skali, dob se mjeri u godinama, a srednja razina glukoze u mg/dL, bilo kakva usporedba je besmislena trenutačno, no prije same usporedbe potrebno je utvrditi da je model valjan, u suprotnom su bilo kakvi postignuti zaključci nevažeći.

Provest ćemo LR test kako bismo to utvrdili, postavljamo hipoteze:
H0: niti jedan od odabranih atributa (dob, razina glukoze, BMI, hipertenzija) ne utječe na pojavu moždanog udara
H1: barem jedan od odabranih atributa ima značajan utjecaj na izgled pojave moždanog udara

LR test ćemo provesti uspoređujući dobiveni model s "null" modelom, null model sadrži samo slobodni koeficijent, odnosno ne ovisi niti o jednom atributu podataka osim zavisne varijable. Ako uočimo značajnu p-vrijednost na izlazu testa, imat ćemo potvrdu da je dodatak prediktora u model imao bitan utjecaj.

```{r}
null_model <- glm(stroke ~ 1,
                  data = reg_data,
                  family = binomial)

anova(null_model, model, test = "Chisq")
```

Vidimo iz rezultata da je p-vrijednost manja od 0.001 što nam daje izrazitu značajnost i znači da možemo odbaciti nultu hipotezu. Dakle zaključili smo da je ovaj model valjan na visokoj razini značajnosti tj. da je barem jedan od ovih faktora imao veliki utjecaj na izgled pojave moždanog udara.

Sada možemo prijeći na glavno pitanje, koji od ovih čimbenika najbolje predviđa pojavu moždanog udara?

Najlakše ćemo to utvrditi usporedbom omjera izglednosti nad standardiziranim varijablama. Prvo ćemo dakle ponoviti izradu modela sa sva tri numerička čimbenika standardizirana pa dobivene koeficijente eksponirat i usporedit, onaj s najvećom vrijednosti najbolje predviđa. Sa standardiziranim varijablama neće biti ranijeg problema gdje je svaka varijabla u svojoj mjernoj jedinici i skali, sve će se varijabli mjeriti u standardnim devijancama, odnosno umjesto da gledamo npr. dob u godinama gledat ćemo je kao udaljenost od vlastitog prosjeka izražena u vlastitoj standardnoj devijanci.

```{r}
# Transformiramo podatke
reg_data <- reg_data %>%
  mutate(
    age_z = (age - mean(age)) / sd(age),
    glucose_z = (avg_glucose_level - mean(avg_glucose_level)) / sd(avg_glucose_level),
    bmi_z = (bmi - mean(bmi)) / sd(bmi)
  )

# Stvaramo novi model
model_std <- glm(stroke ~ age_z + glucose_z + bmi_z + hypertension,
             data = reg_data,
             family = binomial)

summary(model_std)

exp(coef(model_std))
```

Ovdje sada jasno vidimo da je najutjecajniji čimbenik dob, za svaku standardnu devijancu od srednje vrijednosti, množimo izglednost moždanog udara s 4.98. Nakon dobi dolazi hipertenzija koja nije standardizirana zbog svoje binarnosti pa se manje precizno ovdje uspoređuje te nakon nje razina glukoze i konačno BMI koji ima vrlo slab utjecaj i u testu ne značajan utjecaj. Ovo nam se slaže s očekivanjima, moždani udari su najzastupljeniji među starijim osobama, čimbenici poput hipertenzije i visoke prosječne razine glukoze mogu značajno utjecati na pojavu moždanog udara, no nisu presudni, s druge strane, BMI je općenito nestabilna mjera te evidentno nema veliki učinak.

Zaključujemo da je dob čimbenik koji najbolje predviđa nastanak moždanog udara.

## 4. pitanje: Postoji li povezanost između statusa pušenja i nastanka moždanog udara?

Cilj ovog zadatka je ispitati postoji li statistički značajna povezanost između statusa pušenja i pojave moždanog udara. Pušenje je poznati rizični faktor za kardiovaskularne bolesti, stoga je važno provjeriti postoji li povezanost između ove navike i nastanka moždanog udara u promatranom skupu podataka.


### Vizualizacija
Prije provođenja testa, provodimo analizu kako bismo stekli uvid u strukturu podataka i potencijalne obrasce između statusa pušenja i pojave moždanog udara.


```{r}
smoking_freq <- table(stroke_data$smoking_status)
smoking_freq

barplot(
  smoking_freq,
  col = "lightgray",
  main = "Distribucija statusa pušenja",
  xlab = "Status pušenja",
  ylab = "Broj ispitanika"
)


```
Budući da su skupine prema statusu pušenja različitih veličina, rezultati testa mogu biti pod većim utjecajem dominantnih kategorija. Manje skupine mogu imati izraženije razlike u udjelima, ali njihov doprinos testnoj statistici može biti manji. Stoga je uz p-vrijednost važno analizirati i relativne udjele te grafičke prikaze podataka.


```{r}
tab <- table(stroke_data$smoking_status, stroke_data$stroke)

# Udio moždanog udara po kategoriji pušenja
prop_tab <- prop.table(tab, margin = 1)
prop_tab

```

```{r}
barplot(
  prop_tab,
  beside = TRUE,
  col = c("blue", "tomato", "green", "gray"),
  legend = TRUE,
  main = "Udio moždanog udara prema statusu pušenja",
  xlab = "Status pušenja",
  ylab = "Udio"
)
```

Vidljivo je da u svim kategorijama pušenja većinu čine ispitanici bez moždanog udara. Udio moždanog udara razlikuje se između kategorija statusa pušenja. Najviši udio moždanog udara uočava se kod osoba koje su ranije pušile (formerly smoked), dok je najniži udio zabilježen u kategoriji (Unknown).

Ove razlike u udjelima sugeriraju moguću povezanost između statusa pušenja i pojave moždanog udara, no za potvrdu ove pretpostavke potrebno je provesti statistički test.


### Statistički test

Za ispitivanje nezavisnosti varijable pušenja i moždanog udara, koristimo hi-kvadrat test nezavisnosti za kategorijske podatke. Hi-kvadrat test omogućuje ispitivanje hipoteze o nezavisnosti promatranih varijabli, što ga čini prikladnim za analizu statusa pušenja i pojave moždanog udara u ovom skupu podataka.



Za ovaj test postavljamo: 
H0: smoking_status i stroke su nezavisni
H1: smoking_status i stroke su zavisni

Postavljamo razinu značajnosti na 0.5

Prije testa trebamo napraviti kontingencijsku tablicu.

Kontingencijskim tablicama prikazujemo frekvencije uzorka dobivenog mjerenjem dvodimenzionalnog kategorijskog ili diskretnog numeričkog obilježja (X, Y )


```{r}
# Kontingencijska tablica

tbl <- table(
  stroke_data$smoking_status,
  stroke_data$stroke
)

tbl


```
U tablici frekvencija vidimo da je kategorija 'Unknown' prisutna u velikom broju, a ne predstavlja stvarni status pušenja, s toga će mo ju maknuti iz tablice da ne bi loše utjecala na test.

Kopirajmo najprije podatke u novi data.frame kako ne bi promijenili prave vrijednosti.

```{r}
strokeData_copy = data.frame(stroke_data)
```
Kako bi lakše baratali sa varijablom "smoking_status", možemo je pretvoriti u "character" tip podatka.

```{r}
strokeData_copy['smoking_status'] <- sapply(strokeData_copy['smoking_status'], as.character);

```
Izbacimo status 'Unknown' iz tablice

```{r}
tab <- table(strokeData_copy[strokeData_copy$smoking_status == "formerly smoked" | strokeData_copy$smoking_status == "never smoked" | strokeData_copy$smoking_status == "smokes",]$smoking_status, 
            strokeData_copy[strokeData_copy$smoking_status == "formerly smoked" | strokeData_copy$smoking_status == "never smoked" | strokeData_copy$smoking_status == "smokes",]$stroke)
tab
```

```{r}
# Dodamo sume redaka i stupaca
added_margins_tbl = addmargins(tab)
added_margins_tbl
```


Iz tablice je izostavljena kategorija 'Unknown' jer ne predstavlja stvarni status pušenja te može loše utjecati na test.

Pretpostavka testa je da očekivana frekvencija pojedinog razreda mora biti veća ili jednaka 5.

```{r}
for (col_names in colnames(added_margins_tbl)){
  for (row_names in rownames(added_margins_tbl)){
    if (!(row_names == 'Sum' | col_names == 'Sum') ){
      cat('Očekivane frekvencije za razred ',col_names,'-',row_names,': ',(added_margins_tbl[row_names,'Sum'] * added_margins_tbl['Sum',col_names]) / added_margins_tbl['Sum','Sum'],'\n')
    }
  }
}
```
Pošto su sve frekvencije veće od 5, pretpostavka testa je zadovoljena i možemo nastaviti sa testom.

```{r}
# Hi-kvadrat test
test <- chisq.test(tab, correct=F)
test
```


Dobivena p-vrijednost iznosi p = 0.003285. Budući da je p-vrijednost manja od razine značajnosti α = 0.05, odbacujemo nultu hipotezu u korist alternativne hipoteze. Zaključujemo da postoji statistički značajna povezanost između statusa pušenja i pojave moždanog udara.



### Zaključak

Rezultati analize pokazuju da je status pušenja statistički značajno povezan s pojavom moždanog udara u promatranom skupu podataka. Ovaj rezultat u skladu je s medicinskim spoznajama da pušenje povećava
rizik od kardiovaskularnih i cerebrovaskularnih bolesti.

Iako se na temelju ovih podataka ne može zaključiti uzročnost, dobiveni rezultati naglašavaju važnost prestanka pušenja kao preventivne mjere protiv moždanog udara.










